    ...
    def _build_system_prompt(cls, document_type: str) -> str:
        ...
        type_instructions = {
            "invoice": (
                ...
                "- pricing_type (string or null: 'tax_exclusive' if line prices exclude tax, 'tax_inclusive' if line prices include tax/VAT/GST)\n"
                "- subtotal (float or null: NET pre-tax subtotal / Net worth / taxable base)\n"
                "- taxable_amount (float or null: explicit taxable value if reported separately)\n"
                "- tax_amount (float or null: total tax / VAT / GST amount)\n"
                "- tax_rate (float or null, e.g. 0.18 for 18%)\n"
                "- tax_components (list of objects: e.g. [{'name': 'CGST', 'rate': 0.09, 'amount': 523.36}, {'name': 'SGST', 'rate': 0.09, 'amount': 523.36}])\n"
                "- discount_amount (float or null: overall or order-level monetary discount)\n"
                "- discount_scope (string or null: 'invoice_level' or 'line_item_summary')\n"
                "- shipping_amount (float or null)\n"
                "- round_off_amount (float or null: explicit round-off adjustment)\n"
                "- total_amount (float or null: GROSS total payable / Grand total)\n"
                "- currency (string ISO code e.g. USD, EUR, INR or null)\n"
                "- payment_terms (string or null)\n"
                "- line_items (list of objects: description, quantity, unit_price, discount_percent, discount_amount, tax_rate, tax_amount, total_price, gross_amount, is_tax_inclusive, item_code, source_snippet, page_number)\n"
                "  * unit_price: unit rate/price (before discount).\n"
                "  * discount_percent: explicit percentage discount on this line (e.g. 99.0 for 99%), or null if none.\n"
                "  * discount_amount: explicit monetary discount on this line, or null if none.\n"
                "  * total_price: reported line amount (after line discount).\n"
                "  * gross_amount: GROSS line total including tax if printed; null if not printed.\n"
            ...
            "balance_sheet": (
                ...
                "- total_liabilities_and_equity (float or null: Total Capital and Liabilities or Total Liabilities & Equity)\n"
                "- total_capital_and_liabilities (float or null: Explicit Total Capital and Liabilities)\n"
                "- line_items (list of objects: category, item_name, amount, source_snippet, page_number. Set category to 'asset', 'liability', 'capital', or 'equity'. Exclude footnote/off-balance-sheet items like contingent liabilities and bills for collection from totals)\n"
                "- additional_fields (object: any other line items, footnotes, or disclosures)\n"
            ),
            "profit_and_loss": (
